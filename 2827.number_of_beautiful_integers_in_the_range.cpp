...
public:
  int numberOfBeautifulIntegers(int low, int high, int k) {
    ...
    function<int(int, int, int, bool, bool)> dfs = [&](int pos, int mod, int diff, bool lead, bool limit) {
      ...
      if (! lead and ! limit and f[pos][mod][diff] != -1) { return f[pos][mod][diff]; }
      int ans = 0;
      int up = limit ? s[pos] - '0' : 9;
      for (int i = 0; i <= up; ++i) {
        if (i == 0 and lead) { ans += dfs(pos + 1, mod, diff, true, limit and i == up); } else {
          int nxt = diff + (i % 2 == 1 ? 1 : -1);
          ans += dfs(pos + 1, (mod * 10 + i) % k, nxt, false, limit and i == up);
        }
      }
      if (! lead and ! limit) { f[pos][mod][diff] = ans; }
      return ans;
    };
