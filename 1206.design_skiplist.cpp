...
public:
    ...
    bool search(int target) {
        ...
        for (int i = level - 1; ~i; --i) {
            ...
            if (curr->next[i] and curr->next[i]->val == target) return true;
        }
        return false;
    }
    ...
    bool erase(int num) {
        ...
        while (level > 1 and !head->next[level - 1]) --level;
        return ok;
    }

    Node* findClosest(Node* curr, int level, int target) {
        while (curr->next[level] and curr->next[level]->val < target) curr = curr->next[level];
        return curr;
    }

    int randomLevel() {
        int lv = 1;
        while (lv < maxLevel and rand() < p) ++lv;
        return lv;
    }
};
