...
public:
  ...
  bool query(int left, int right, int l, int r, Node *node) {
    ...
    bool v = true;
    if (left <= mid)
      v = v and query(left, right, l, mid, node->left);
    if (right > mid)
      v = v and query(left, right, mid + 1, r, node->right);
    return v;
  }

  void pushup(Node *node) {
    node->v = node->left and node->left->v and node->right and node->right->v;
  }
...
public:
  ...
  RangeModule() { tree = new SegmentTree(); }

  void addRange(int left, int right) { tree->modify(left, right - 1, 1); }

  bool queryRange(int left, int right) { return tree->query(left, right - 1); }

  void removeRange(int left, int right) { tree->modify(left, right - 1, -1); }
};
