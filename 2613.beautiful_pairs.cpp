...
public:
    vector<int> beautifulPair(vector<int>& nums1, vector<int>& nums2) {
        ...
        function<tuple<int, int, int>(int, int)> dfs = [&](int l, int r) -> tuple<int, int, int> {
            ...
            auto t1 = dfs(l, m);
            auto t2 = dfs(m + 1, r);
            if (get<0>(t1) > get<0>(t2) or (get<0>(t1) == get<0>(t2) and (get<1>(t1) > get<1>(t2) or (get<1>(t1) == get<1>(t2) and get<2>(t1) > get<2>(t2))))) {
                swap(t1, t2);
            }
            ...
            for (int i = 0; i < t.size(); ++i) {
                for (int j = i + 1; j < t.size(); ++j) {
                    ...
                    if (d < get<0>(t1) or (d == get<0>(t1) and (pi < get<1>(t1) or (pi == get<1>(t1) and pj < get<2>(t1))))) {
                        t1 = {d, pi, pj};
                    }
                }
            }
